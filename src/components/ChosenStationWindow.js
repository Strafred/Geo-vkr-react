...
export const ChosenStationWindow = ({station, setClickedStation, setTemperature}) => {
  ...
  useEffect(() => {
      console.log("useEffect for channels called");
  ...
  useEffect(() => {
      console.log("useEffect for satellite called");
      ...
      const satelliteDataQuery = `from(bucket: "DREG") |> range(start: -${chosenTime}) |> filter (fn: (r) => r.topic == "DREG/{${station.stationName}}/gnss/nsat") |> map (fn: (r) => ({time: r._time, value: r._value}))`;
      ...
      dataQuery(satelliteDataQuery, setSatellitesXDrawData, setSatellitesYDrawData)
        .then(() => {
      ...
      client.on('message', (topic, payload) => {
        console.log("message: " + payload.toString());
        console.log("topic: " + topic);
        if (topic === `DREG/{${station.stationName}}/gnss/nsat`) {
          const [oldDate, currentDate] = getLastTime(chosenTime);
